...
/// The wire contract for `list_user_leave_requests`.
///
...
/// `supabase/migrations/20260802120000_leave_requests_pagination.sql` lives in
/// this one place.
abstract final class LeaveRequestsRpc {
  static const String listFunction = 'list_user_leave_requests';
  static const String monthsFunction = 'list_user_leave_request_months';
  ...
  /// match against.
  static Map<String, dynamic> params(LeaveRequestsQuery query, {required int offset, required int limit}) {
  ...
  static PagedResult<UserRequestRow> parse(dynamic response) {
    ...
    final items =
        (row['rows'] as List? ?? const []).map<UserRequestRow>((entry) {
          final wrapper = Map<String, dynamic>.from(entry as Map);
          final payload = Map<String, dynamic>.from(wrapper['payload'] as Map);
          return wrapper['row_kind'] == 'leave'
              ? LeaveRequestRow(LeaveRequestModel.fromJson(payload))
              : LeaveCancellationRow(LeaveCancellationRequestModel.fromJson(payload));
        }).toList();
  ...
  /// Parses the `list_user_leave_request_months` result into first-of-month
