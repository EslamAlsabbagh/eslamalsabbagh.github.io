...
/// The wire contract for `list_user_hr_letter_requests`.
///
...
/// has to agree with
/// `supabase/migrations/20260804140000_hr_letter_requests_pagination.sql` lives
/// in this one place.
abstract final class HrLetterRequestsRpc {
  static const String listFunction = 'list_user_hr_letter_requests';
  static const String monthsFunction = 'list_user_hr_letter_request_months';
  ...
  /// match against.
  static Map<String, dynamic> params(HrLetterRequestsQuery query, {required int offset, required int limit}) {
  ...
  ///
  /// Flat payloads, like overtime and missing punch: HR letters have no
  /// cancellation sibling table, so `rows` needs no `row_kind` wrapper to
  /// unwrap and no sealed row type to build.
  static PagedResult<HrLetterRequestModel> parse(dynamic response) {
    ...
    final items =
        (row['rows'] as List? ?? const [])
            .map<HrLetterRequestModel>(
              (entry) => HrLetterRequestModel.fromJson(Map<String, dynamic>.from(entry as Map)),
            )
            .toList();
  ...
  /// Parses the `list_user_hr_letter_request_months` result into first-of-month
