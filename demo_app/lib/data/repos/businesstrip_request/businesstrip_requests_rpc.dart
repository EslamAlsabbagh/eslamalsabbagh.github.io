...
/// The wire contract for `list_user_businesstrip_requests`.
///
...
abstract final class BusinesstripRequestsRpc {
  static const String listFunction = 'list_user_businesstrip_requests';
  static const String monthsFunction = 'list_user_businesstrip_request_months';
  ...
  /// match against.
  static Map<String, dynamic> params(BusinesstripRequestsQuery query, {required int offset, required int limit}) {
  ...
  static PagedResult<BusinesstripRequestRow> parse(dynamic response) {
    ...
    final items =
        (row['rows'] as List? ?? const []).map<BusinesstripRequestRow>((entry) {
          final wrapper = Map<String, dynamic>.from(entry as Map);
          final payload = Map<String, dynamic>.from(wrapper['payload'] as Map);
          return wrapper['row_kind'] == 'trip'
              ? BusinesstripTripRow(BusinesstripRequestModel.fromJson(payload))
              : BusinesstripCancellationRow(BusinesstripCancellationRequestModel.fromJson(payload));
        }).toList();
  ...
  /// Parses the `list_user_businesstrip_request_months` result into
