...
abstract class OvertimeRequestRepo {
  Future<int> submitOvertimeRequest(OvertimeRequestModel request);

  /// One page of the overtime list.
  ///
  ...
  Future<PagedResult<OvertimeRequestModel>> getOvertimeRequestsPage(
  ...
  Future<List<DateTime>> getOvertimeRequestMonths(OvertimeRequestScope scope);
  ...
  Future<bool> hasAnyRequests(OvertimeRequestScope scope, int userCode);

  // ── LEGACY: unpaginated, whole-list reads ──────────────────────────────────
  // Prefer [getOvertimeRequestsPage]. These fetch every matching row and are
  ...
  // UserOvertimeRequestsBloc still calls them, and so do the dashboard
  // summaries (pending_requests_summary_bloc.dart:70,
  // my_requests_summary_bloc.dart:55) and the sidebar's menu gate
  // (user_bloc.dart:152-157), which only want integer counts.

  /// Legacy whole-list read. See the LEGACY note above.
  Future<List<OvertimeRequestModel>> getMyOvertimeRequests(int userCode);

  /// Legacy whole-list read. See the LEGACY note above.
  Future<List<OvertimeRequestModel>> getTeamRequests(int userCode);

  /// Legacy whole-list read. See the LEGACY note above.
  Future<List<OvertimeRequestModel>> getProcessedRequests(int approverCode);

  Future<void> approveRequest(int requestId, String currentApprover, int approverCode);
  Future<void> declineRequest(int requestId, String reason, int userCode, String currentApprover, int approverCode);
  Future<List<OvertimeRequestModel>> getRequestsByMonth(int userCode, DateTime month);
