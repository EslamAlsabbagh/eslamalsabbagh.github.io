...
/// The wire contract for `list_user_missingpunch_requests`.
///
...
/// `supabase/migrations/20260804120000_missingpunch_requests_pagination.sql`
/// lives in this one place.
abstract final class MissingpunchRequestsRpc {
  static const String listFunction = 'list_user_missingpunch_requests';
  static const String monthsFunction = 'list_user_missingpunch_request_months';
  ...
  ///
  /// Simpler than the leave and business-trip equivalents: missing punch has no
  ...
  static PagedResult<MissingpunchingRequestModel> parse(dynamic response) {
    ...
    final items =
        (row['rows'] as List? ?? const [])
            .map<MissingpunchingRequestModel>(
              (entry) => MissingpunchingRequestModel.fromJson(Map<String, dynamic>.from(entry as Map)),
            )
            .toList();
  ...
  /// Parses the `list_user_missingpunch_request_months` result into
