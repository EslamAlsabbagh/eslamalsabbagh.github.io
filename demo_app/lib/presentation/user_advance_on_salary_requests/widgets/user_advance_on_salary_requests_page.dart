...
class UserAdvanceOnSalaryRequestsPage extends StatelessWidget {
  const UserAdvanceOnSalaryRequestsPage({super.key});
    ...
    return BlocProvider<UserAdvanceOnSalaryRequestsBloc>(
      create: (context) {
        ...
        return UserAdvanceOnSalaryRequestsBloc(advanceRepo, workflowService: workflowService)
          ..add(loadAdvanceRequestsEvent(context.read<UserBloc>().state.user?.id ?? 0, RequestSourceType.myRequests));
      },
      child: const UserAdvanceOnSalaryRequestsContent(sourceType: RequestSourceType.myRequests),
    );
  }
}
