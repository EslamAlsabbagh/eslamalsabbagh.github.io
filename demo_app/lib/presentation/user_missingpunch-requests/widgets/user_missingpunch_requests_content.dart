...
class UserMissingpunchRequestsContent extends StatefulWidget {
  final RequestSourceType? sourceType;
  const UserMissingpunchRequestsContent({super.key, this.sourceType});

  @override
  State<UserMissingpunchRequestsContent> createState() => _UserMissingpunchRequestsContentState();
}

class _UserMissingpunchRequestsContentState extends State<UserMissingpunchRequestsContent> with RequestFiltersMixin {
  int _rowsPerPage = PaginatedDataTable.defaultRowsPerPage;
  int? _sortColumnIndex;
  bool _sortAscending = true;
  late _MissingpunchRequestsDataSource _dataSource;

  // Track previous state to avoid unnecessary data source recreation
  List<MissingpunchingRequestModel>? _previousRequests;

  // ── Server-paged state (unused when the flag is off) ──────────────────────
  final PaginatorController _paginator = PaginatorController();
  _MissingpunchRequestsAsyncSource? _asyncDataSource;
  MissingpunchRequestsQuery? _query;
  ...
  MissingpunchRequestsQuery _buildQuery() => MissingpunchRequestsQuery(
    scope: _effectiveSourceType.scope,
    search: searchQuery,
    status: statusFilter,
    month: selectedMonth,
    sortKey: _query?.sortKey ?? MissingpunchRequestSortKey.createdAt,
  ...
  void _applyQuery(MissingpunchRequestsQuery next) {
  ...
  void onFiltersChanged() {
    if (!FeatureFlags.serverPagedMissingpunchRequests) return;
  ...
  void _sortColumn(MissingpunchRequestSortKey key, int columnIndex, bool ascending) {
    setState(() {
  ...
  void _sort<T>(Comparable<T> Function(MissingpunchingRequestModel d) getField, int columnIndex, bool ascending) {
  ...
  void _onSort<T>(
    MissingpunchRequestSortKey serverKey,
    Comparable<T> Function(MissingpunchingRequestModel d) legacyGetField,
    int columnIndex,
    bool ascending,
  ) {
  ...
  /// Whether [request] overlaps a same-user business trip.
  ///
  /// On the paged path this is a per-row flag computed by the RPC; the
  /// whole-list scan behind `state.conflictingRequestIds` cannot see rows
  /// outside the page currently in memory.
  bool _hasConflict(MissingpunchingRequestModel request, Set<int> legacyConflictIds) =>
      FeatureFlags.serverPagedMissingpunchRequests
          ? request.hasBusinesstripConflict
          : (request.id != null && legacyConflictIds.contains(request.id));

  void _refreshRequests() {
    final userCode = context.read<UserBloc>().state.user?.id ?? 0;
    context.read<UserMissingpunchRequestsBloc>().add(
      loadMissingpunchRequestsEvent(userCode, _effectiveSourceType),
    );
  }

  List<MissingpunchingRequestModel> _filterRequests(List<MissingpunchingRequestModel> requests) {
  ...
  Widget _buildRequestDetailsContent(MissingpunchingRequestModel request, BuildContext context) {
    return SingleChildScrollView(
      ...
      child: Column(
        ...
        children: [
          ...
          // Same-day business-trip overlap warning, shown to approvers (n1/n2/hr).
          if (_hasConflict(request, context.read<UserMissingpunchRequestsBloc>().state.conflictingRequestIds))
            Padding(
              ...
              child: Row(
                ...
                children: [
                  ...
                  Flexible(
                    child: Text(
                      AppLocalizations.of(context)!.sameDayBusinessTripWarning,
          ...
          Row(
            children: [
              ConstrainedBox(
                ...
                child: Row(
                  children: [
                    Text("${AppLocalizations.of(context)!.time}: ", style: TextStyle(fontWeight: FontWeight.bold)),
                    Text(request.time != null ? DateFormat('HH:mm').format(request.time!) : ''),
                  ],
                ),
              ),
              ConstrainedBox(
                ...
                child: Row(
                  children: [
                    Text("${AppLocalizations.of(context)!.type}: ", style: TextStyle(fontWeight: FontWeight.bold)),
                    Text(request.getLocalizedType(context)),
                  ],
                ),
              ),
            ],
  ...
  void _showApprovalConfirmationDialog(MissingpunchingRequestModel request) {
    showDialog(
      context: context,
  ...
  Widget build(BuildContext context) {
    return BlocConsumer<UserMissingpunchRequestsBloc, UserMissingpunchRequestsState>(
      listener: (context, state) {
        ...
        if (state.cancelStatus == Status.success) {
          ...
          context.read<UserMissingpunchRequestsBloc>().add(const ResetCancelStatus());
      ...
      builder: (context, state) {
        ...
        // re-add it.
        if (!FeatureFlags.serverPagedMissingpunchRequests && state.status == Status.loading) {
        ...
        // MissingpunchingRequestRepoImpl.hasAnyRequests), so the two should
        // never disagree. The table is holding the evidence, so a populated one
        // always wins over a probe that says otherwise.
                ...
                child: Column(
                  children: [
                    ...
                    SizedBox(
                      ...
                      child: Center(
                        child: Text(
                          AppLocalizations.of(context)!.noMissingPunchingRequestsFound,
        ...
        if (FeatureFlags.serverPagedMissingpunchRequests) {
          ...
          _asyncDataSource ??= _MissingpunchRequestsAsyncSource(
            repo: context.read<MissingpunchingRequestsRepo>(),
            ...
            rowSourceFactory:
                (pageRows, query) => _makeDataSource(
                  ...
                  // _MissingpunchRequestsAsyncSource.isStale — which withholds
                  // these rows until the query catches up — it guarantees the
                  // cells and the header always describe the same tab.
                  shouldShowActionColumn: query.scope != MissingpunchRequestScope.processed,
                ),
          );
  ...
  /// Builds the row source. Shared by both paging modes so the ~500 lines of
  /// cell layout, dialogs and action buttons live in exactly one place.
  ///
  ...
  _MissingpunchRequestsDataSource _makeDataSource({
    required List<MissingpunchingRequestModel> requests,
    required bool shouldShowActionColumn,
    bool isLoading = false,
    Set<int> conflictingRequestIds = const {},
  }) {
    return _MissingpunchRequestsDataSource(
      requests,
      isLoading: isLoading,
      context: context,
        ...
        showDialog(
          ...
          builder: (childContext) {
            ...
            return BlocProvider.value(
              value: context.read<UserMissingpunchRequestsBloc>(),
              child: AlertDialog(
      ...
      onCancel: (request) {
        context.read<UserMissingpunchRequestsBloc>().add(CancelMissingpunchRequest(request.id!));
      },
      onRemove: (request) {
        if (widget.sourceType == RequestSourceType.teamRequests) {
          context.read<UserMissingpunchRequestsBloc>().add(RemoveMissingpunchRequestForN1(request.id!));
        } else {
          context.read<UserMissingpunchRequestsBloc>().add(RemoveMissingpunchRequest(request.id!));
        }
      },
    );
  }
    ...
    return [
      if (widget.sourceType == RequestSourceType.teamRequests)
        DataColumn2(
          ...
          onSort:
              (i, asc) =>
                  _onSort<String>(MissingpunchRequestSortKey.userId, (d) => d.userId.toString(), i, asc),
        ),
      if (widget.sourceType == RequestSourceType.teamRequests)
        DataColumn2(
          size: ColumnSize.L,
          ...
          onSort:
              (i, asc) => _onSort<String>(
                MissingpunchRequestSortKey.employeeName,
      ...
      DataColumn2(
        ...
        onSort:
            (i, asc) => _onSort<DateTime>(MissingpunchRequestSortKey.createdAt, (d) => d.createdAt!, i, asc),
      ),
      DataColumn2(
        size: ColumnSize.S,
        label: Center(child: Text(AppLocalizations.of(context)!.date, style: TextStyle(fontWeight: FontWeight.bold))),
        onSort: (i, asc) => _onSort<DateTime>(MissingpunchRequestSortKey.date, (d) => d.date!, i, asc),
      ),
      DataColumn2(
        size: ColumnSize.S,
        label: Center(child: Text(AppLocalizations.of(context)!.time, style: TextStyle(fontWeight: FontWeight.bold))),
        onSort: (i, asc) => _onSort<DateTime>(MissingpunchRequestSortKey.time, (d) => d.time!, i, asc),
      ),
      DataColumn2(
        size: ColumnSize.S,
        label: Center(child: Text(AppLocalizations.of(context)!.type, style: TextStyle(fontWeight: FontWeight.bold))),
        onSort: (i, asc) => _onSort<String>(MissingpunchRequestSortKey.type, (d) => d.type ?? '', i, asc),
      ),
      DataColumn2(
        size: ColumnSize.S,
        label: Center(child: Text(AppLocalizations.of(context)!.status, style: TextStyle(fontWeight: FontWeight.bold))),
        onSort: (i, asc) => _onSort<String>(MissingpunchRequestSortKey.status, (d) => d.status!, i, asc),
      ),
      DataColumn2(
        size: ColumnSize.S,
        label: Center(
        ...
        onSort:
            (i, asc) =>
                _onSort<String>(MissingpunchRequestSortKey.currentApprover, (d) => d.currentApprover!, i, asc),
      ),
      if (showActionColumn)
        DataColumn2(
          size: ColumnSize.L,
...
class _MissingpunchRequestsDataSource extends DataTableSource {
  final List<MissingpunchingRequestModel> _requests;
  final bool isLoading;
  final BuildContext context;
  final RequestSourceType? sourceType;
  final Function(MissingpunchingRequestModel, {bool fromDetails})? onApproved;
  final Function(MissingpunchingRequestModel request, {bool fromDetails})? onDeclined;
  final Function(MissingpunchingRequestModel)? onCancel;
  final Function(MissingpunchingRequestModel)? onRemove;
  final _UserMissingpunchRequestsContentState parentState;
  final bool shouldShowActionColumn;

  /// Ids of missing-punch requests overlapping a same-user business-trip day;
  /// their rows are tinted red.
  final Set<int> conflictingRequestIds;

  _MissingpunchRequestsDataSource(
    this._requests, {
  ...
  /// business-trip request, otherwise the table default.
  ///
  /// Which source answers that depends on the paging mode — see
  /// [_UserMissingpunchRequestsContentState._hasConflict].
  WidgetStateProperty<Color?>? _conflictRowColor(MissingpunchingRequestModel request) {
    if (parentState._hasConflict(request, conflictingRequestIds)) {
      return WidgetStateProperty.resolveWith<Color?>((states) => Colors.red[50]);
    }
    return null;
  }

  void sort<T>(Comparable<T> Function(MissingpunchingRequestModel d) getField, bool ascending) {
    _requests.sort((a, b) {
  ...
  DataRow? getRow(int index) {
    ...
    return DataRow(
      ...
      cells: [
        ...
        DataCell(Center(child: Text(dateFormatter.format(request.date!)))),
        DataCell(Center(child: Text(request.time != null ? timeFormatter.format(request.time!) : ''))),
        DataCell(Center(child: Text(request.getLocalizedType(context)))),
        ...
        if (shouldShowActionColumn)
          (sourceType == RequestSourceType.teamRequests && request.isPending)
              ? DataCell(
                Center(
                  child: RowApproveDeclineActions<UserMissingpunchRequestsBloc, UserMissingpunchRequestsState>(
                  ...
                  child:
                      request.isActionable && sourceType == RequestSourceType.myRequests
                          ? AppButton(
                            ...
                            onPressed: () => _showCancelConfirmationDialog(request),
                            label: AppLocalizations.of(context)!.cancel,
  ...
  void _showRemoveConfirmationDialog(MissingpunchingRequestModel request) {
    showDialog(
      context: context,
  ...
  void _showCancelConfirmationDialog(MissingpunchingRequestModel request) {
    showDialog(
      context: context,
      builder: (BuildContext dialogContext) {
        return AlertDialog(
  ...
  void _showCancelConfirmationDialogFromDetails(
    MissingpunchingRequestModel request,
    BuildContext detailsDialogContext,
  ) {
    showDialog(
      context: context,
      builder: (BuildContext dialogContext) {
  ...
  void _showRequestDetailsDialog(MissingpunchingRequestModel request, BuildContext context) {
    showDialog(
      context: context,
...
/// Fetches one page at a time from `list_user_missingpunch_requests`.
///
/// Row *rendering* is not duplicated here: [getRows] builds a throwaway
/// [_MissingpunchRequestsDataSource] over the page it just fetched and calls its
...
/// survives.
class _MissingpunchRequestsAsyncSource extends AsyncDataTableSource {
  _MissingpunchRequestsAsyncSource({
    required this.repo,
    required MissingpunchRequestsQuery query,
    required this.rowSourceFactory,
  }) : _query = query;

  final MissingpunchingRequestsRepo repo;
  ...
  final _MissingpunchRequestsDataSource Function(
    List<MissingpunchingRequestModel> pageRows,
    MissingpunchRequestsQuery query,
  )
  rowSourceFactory;

  MissingpunchRequestsQuery _query;
  MissingpunchRequestsQuery get query => _query;
  ...
  MissingpunchRequestsQuery? _renderedQuery;
  ...
  bool setQuery(MissingpunchRequestsQuery next) {
  ...
  Future<AsyncRowsResponse> getRows(int startIndex, int count) async {
    ...
    final query = _query;
    final page = await repo.getMissingpunchRequestsPage(query, offset: startIndex, limit: count);
