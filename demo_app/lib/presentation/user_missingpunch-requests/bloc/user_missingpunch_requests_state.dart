...
final class UserMissingpunchRequestsState extends BaseState {
  const UserMissingpunchRequestsState({
  ...
  /// (`FeatureFlags.serverPagedMissingpunchRequests == false`); empty otherwise,
  /// because the paged table holds its rows in its own `AsyncDataTableSource`.
  final List<MissingpunchingRequestModel> requests;
  ...
  /// Ids of missing-punch requests that fall on a same-user business-trip day.
  ///
  /// LEGACY path only. On the paged path the overlap rides on each row as
  /// [MissingpunchingRequestModel.hasBusinesstripConflict], because the
  /// whole-list scan this set came from cannot see rows outside the page in
  /// memory.
  final Set<int> conflictingRequestIds;

  UserMissingpunchRequestsState copyWith({
    Status? status,
    Failure? failure,
    List<MissingpunchingRequestModel>? requests,
    bool? hasAnyRequests,
    ...
    return UserMissingpunchRequestsState(
