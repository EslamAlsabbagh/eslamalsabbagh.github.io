...
class UserOvertimeRequestsPage extends StatelessWidget {
  const UserOvertimeRequestsPage({super.key});

  @override
  Widget build(BuildContext context) {
    return BlocProvider(
      create:
          (_) => UserOvertimeRequestsBloc(context.read<OvertimeRequestRepo>())
            ..add(loadOvertimeRequestsEvent(context.read<UserBloc>().state.user?.id ?? 0, RequestSourceType.myRequests)),
      child: UserOvertimeRequestsContent(),
    );
  }
}
