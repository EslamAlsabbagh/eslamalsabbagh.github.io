...
class MyRecentlyProcessedRequestsWidget extends StatefulWidget {
  final bool embedded;

  const MyRecentlyProcessedRequestsWidget({super.key, this.embedded = false});

  @override
  State<MyRecentlyProcessedRequestsWidget> createState() => _MyRecentlyProcessedRequestsWidgetState();
}

class _MyRecentlyProcessedRequestsWidgetState extends State<MyRecentlyProcessedRequestsWidget> {
  ...
  Widget build(BuildContext context) {
    return BlocBuilder<MyRequestsSummaryBloc, MyRequestsSummaryState>(
      builder: (context, state) {
        ...
        if (!state.summary.hasRecentlyProcessedRequests) return const SizedBox.shrink();
        ...
        if (widget.embedded) {
          return Padding(
            ...
            child: Column(
              ...
              children: [
                scrollableItems,
                if (state.summary.activeRecentlyProcessedTypesCount > 4) _buildScrollIndicator(),
              ],
            ),
          );
        }
              ...
              children: [
                Row(
                  ...
                  children: [
                    ...
                    Expanded(
                      child: Text(
                        AppLocalizations.of(context)!.myRecentlyProcessedRequests,
                ...
                Container(
                  ...
                  child: Text(
                    '${AppLocalizations.of(context)!.total}: ${state.summary.totalRecentlyProcessed}',
                    textAlign: TextAlign.center,
      ...
      child: ScrollConfiguration(
        ...
        child: SingleChildScrollView(
          ...
          child: Padding(
            ...
            child: Column(
              children: [
                if (summary.recentlyProcessedLeaveRequests > 0)
                  _buildSummaryItem(
                    context,
                    icon: Icons.back_hand_outlined,
                    label: l.leaveRequests,
                    count: summary.recentlyProcessedLeaveRequests,
                    color: Colors.blue,
                    onTap: () => _navigate(context, 'leave'),
                  ),
                if (summary.recentlyProcessedOvertimeRequests > 0)
                  _buildSummaryItem(
                    context,
                    icon: Icons.access_time,
                    label: l.overtimeRequests,
                    count: summary.recentlyProcessedOvertimeRequests,
                    color: Colors.orange,
                    onTap: () => _navigate(context, 'overtime'),
                  ),
                if (summary.recentlyProcessedBusinessTripRequests > 0)
                  _buildSummaryItem(
                    context,
                    icon: Icons.mode_of_travel_outlined,
                    label: l.businessTripRequests,
                    count: summary.recentlyProcessedBusinessTripRequests,
                    color: Colors.green,
                    onTap: () => _navigate(context, 'businessTrip'),
                  ),
                if (summary.recentlyProcessedMissingPunchRequests > 0)
                  _buildSummaryItem(
                    context,
                    icon: Icons.fingerprint,
                    label: l.missingPunchRequests,
                    count: summary.recentlyProcessedMissingPunchRequests,
                    color: Colors.purple,
                    onTap: () => _navigate(context, 'missingPunch'),
                  ),
                if (summary.recentlyProcessedAdvanceOnSalaryRequests > 0)
                  _buildSummaryItem(
                    context,
                    icon: Icons.account_balance_wallet_outlined,
                    label: l.advanceOnSalaryRequests,
                    count: summary.recentlyProcessedAdvanceOnSalaryRequests,
                    color: Colors.teal,
                    onTap: () => _navigate(context, 'advanceOnSalary'),
                  ),
                if (summary.recentlyProcessedDisciplinaryActionRequests > 0)
                  _buildSummaryItem(
                    context,
                    icon: Icons.gavel_outlined,
                    label: l.disciplinaryActionRequests,
                    count: summary.recentlyProcessedDisciplinaryActionRequests,
                    color: Colors.red,
                    onTap: () => _navigate(context, 'disciplinaryAction'),
                  ),
                if (summary.recentlyProcessedHrLetterRequests > 0)
                  _buildSummaryItem(
                    context,
                    icon: Icons.article_outlined,
                    label: l.hrLetterRequests,
                    count: summary.recentlyProcessedHrLetterRequests,
                    color: Colors.indigo,
                    onTap: () => _navigate(context, 'hrLetter'),
                  ),
                if (summary.recentlyProcessedSwapRequests > 0)
                  _buildSummaryItem(
                    context,
                    icon: Icons.swap_horiz,
                    label: AppLocalizations.of(context)!.shiftSwapRequests,
                    count: summary.recentlyProcessedSwapRequests,
                    color: Colors.blue,
